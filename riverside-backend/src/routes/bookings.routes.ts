...
router.post("/", requireAuth, async (req, res) => {
  ...
  if (conflictError) return res.status(500).json({ error: "Unable to check availability" });
  if (conflicts?.length) return res.status(409).json({ error: "Resource is already booked for that time" });

  const { data, error } = await supabaseAdmin
    .from("bookings")
    .insert({ ...result.data, member_id: req.user?.id })
    .select("id, member_id, facility_id, equipment_id, start_at, end_at, status, created_at")
    .single();
...
router.patch("/:id/approve", requireAuth, requireRole("staff", "admin"), async (req, res) => {
  ...
  const conflictQuery = supabaseAdmin
    ...
    .limit(1);
  if (booking.facility_id) conflictQuery.eq("facility_id", booking.facility_id);
  if (booking.equipment_id) conflictQuery.eq("equipment_id", booking.equipment_id);
  const { data: conflicts, error: conflictError } = await conflictQuery;
  if (conflictError) return res.status(500).json({ error: "Unable to check availability" });
  if (conflicts?.length) return res.status(409).json({ error: "Resource is already booked for that time" });
  ...
  const { error } = await supabaseAdmin
    ...
    .eq("id", req.params.id);
  if (error) return res.status(500).json({ error: "Unable to approve booking" });
  return res.status(200).json({ message: `Booking ${req.params.id} approved` });
});
...
router.patch("/:id/reject", requireAuth, requireRole("staff", "admin"), async (req, res) => {
  ...
  const { error } = await supabaseAdmin
    ...
    .eq("id", req.params.id);
  if (error) return res.status(500).json({ error: "Unable to reject booking" });
  return res.status(200).json({ message: `Booking ${req.params.id} rejected` });
});
...
router.patch("/:id/cancel", requireAuth, async (req, res) => {
  ...
  return res.status(200).json({ message: `Booking ${req.params.id} cancelled` });
});
